...
- **PASS — reviewer accounts and privacy:** reviewer A is production-renamed to the submitted username `gm-reviewer-a-20260711-1abf65`, and that account now authenticates through `/v1/auth/login`. Reviewer B remains the working `20260730` account.
...
- **RESOLVED — reviewer secret correction:** the secret was submitted through the authenticated Principal API, allowing SecureField's BCrypt setter to hash it; `/v1/auth/login` returned `200` for the submitted account. The current review version remains locked, but no form change was required.
