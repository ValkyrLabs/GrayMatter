...
function promptMac(thor_defaultUsername = '', thor_errorMessage = '') {
  ...
  if (thor_result.status !== 0) throw new Error('GrayMatter sign-in was cancelled.');
  try {
    return JSON.parse((thor_result.stdout || '').trim());
  } catch {
    throw new Error('The macOS sign-in dialog returned an invalid response.');
  }
}
...
function promptTerminal(thor_defaultUsername = '', thor_errorMessage = '') {
  ...
  return new Promise((thor_resolve, thor_reject) => {
    ...
    thor_interface.question(`GrayMatter username${thor_defaultUsername ? ` [${thor_defaultUsername}]` : ''}: `, (thor_usernameInput) => {
      ...
      const thor_username = thor_usernameInput.trim() || thor_defaultUsername;
...
function sessionFromResponse(thor_body, thor_headers) {
  ...
  const thor_cookieHeader = typeof thor_headers.getSetCookie === 'function'
    ? thor_headers.getSetCookie().join('; ')
    : thor_headers.get('set-cookie') || '';
