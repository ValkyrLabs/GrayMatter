...
$thor_node = Get-Command node -ErrorAction SilentlyContinue
if (-not $thor_node) {
  throw 'Node.js 20 or newer is required. Install it from https://nodejs.org/ and rerun this command.'
}
$thor_major = [int]((& node --version).TrimStart('v').Split('.')[0])
if ($thor_major -lt 20) { throw 'GrayMatter requires Node.js 20 or newer.' }

& node (Join-Path $thor_scriptDir 'gm-auth.mjs') keychain
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& node --check (Join-Path $thor_root 'mcp-server/index.js')
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& node --check (Join-Path $thor_scriptDir 'gm-mcp-launcher.mjs')
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
