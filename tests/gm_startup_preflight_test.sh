...
set -euo pipefail
printf '%s\n' "$*" >>"${TEST_API_LOG:?}"
[[ "$*" == "GET /graymatter/omega/capabilities" ]] || exit 2
cat "${TEST_CAPABILITY_FILE:?}"
EOF
...
'
if jq -r tostring "$ready_out" | grep -Eq 'must-not-leak|privateBalance|privateCount'; then
  fail "startup evidence leaked a private capability field"
fi
